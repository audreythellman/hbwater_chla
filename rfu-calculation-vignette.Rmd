...
a) compiled a file with the name `hbwtr_chla_rfu_YEAR.xlsx`
b) uploaded that file to Google Drive
c) updated the *local copy* of `samplinglist_updated.xlsx`
...
We are ready to clean our data and check compatibility with our calculation code chunk! Open up a new R script within `hbwater_chla` and follow the steps. 
...
(input_data_folder <- drive_ls(as_id("1bKuciPhW6x6svEJLHAvcESvMyXbFmbh6"))) #this is the ID for the folder
```
...
```
From here, you will get a list of the input files. Remember the number of the file you are going to clean. In this case, we are going to clean file #1 corresponding to 2020 data. 
...
```{r function out, results='hide'}
chla <- get_chla_data_fr_drive(1, sa = TRUE)
```
*The function will tell you what your last date entered is & the file you downloaded. Be sure this is a later date than the last chla value ran & the correct file*
...
_If the columns don't match, manually change them and re-add your files to **Google Drive**_
...
Next, check if there are any missing data using the following function. Input the year that you are checking in the second line (e.g. if `hbwtr_chla_rfu_2020.xlsx` then type `2020`)
...
The following loads the function, which is a functino of the chla_list (should be `chla`) and the slope (`slp`): 
```{r get function}
