...
Once you have the Rscript open in the hbef_chla project, we can add our packages:
...
We are ready to clean our data and check compatibility with our calculation code chunk! First we are going to locate & load the data:
...
The following loads the function, which is a function of the chla_list (should be `chla`) and the slope (`slp`): 
```{r get function}
